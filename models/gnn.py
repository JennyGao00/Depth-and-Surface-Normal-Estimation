...
class GNNFixedK(torch.nn.Module):
    ...
    def forward(self, pos, old_weights, normals, edge_index, dense_l, stddev, f=None):
        ...
        N = pos.size(0)  # 100000
        K = dense_l.size(1)   # 33
        E = edge_index.size(1)  # 3300000

        rows, cols = edge_index  # 3300000, 3300000
        cart = pos[cols] - pos[rows]  # 3300000, 3
        scale = 0.2 / stddev
        cart = cart * scale
        ppf = compute_prf(pos, normals, edge_index, scale=scale)  # 3300000, 4

        x = torch.cat([cart, old_weights.view(-1, 1), ppf], dim=-1)  # 3300000, 8
        x = self.layer1(x)  # 3300000, 16             # h
        x = x.view(N, K, -1)  # 100000, 33, 16
        global_x = x.mean(1)  # 100000, 16
        if f is not None:
            global_x = torch.cat([global_x, normals.view(-1, 3), f.view(-1, 32)], dim=-1)
        else:
            global_x = torch.cat([global_x, normals.view(-1, 3)], dim=-1)  # 100000, 19
        x_g = self.layerg(global_x)  # 100000, 8      # gamma
        x = torch.cat([x.view(E, -1), x_g[rows]], dim=1)  # 3300000, 16+8=24
        x = self.layer2(x)  # 3300000, 16             # h
        x = x.view(N, K, -1)  # 100000, 33, 16
        global_x = x.mean(1)  # 100000, 16
        x_g = self.layerg2(global_x)  # 100000, 8     # gamma
        x = torch.cat([x.view(E, -1), x_g[rows]], dim=1)  # 3300000, 16+8=24    # gamma
        x = self.layer3(x)  # 3300000, 16             # h
        x = x.view(N, K, -1)  # 100000, 33, 16
        global_x = x.mean(1)  # 100000, 16
        x_g = self.layerg3(global_x)  # 100000, 12    # gamma
        quat = x_g[:, :4]  # (100000,4)
        quat = quat / (quat.norm(p=2, dim=-1) + 1e-8).view(-1, 1)   # (100000, 4)
        mat = QuatToMat.apply(quat).view(-1, 3, 3)  # 100000, 3, 3

        # Kernel application
        x_g = x_g[:, 4:]  # (100000, 8)
        rot_cart = torch.matmul(mat.view(-1, 3, 3)[rows], cart.view(-1, 3, 1)).view(-1, 3)  # (3300000, 3)
        x = torch.cat([x.view(E, -1), x_g[rows], rot_cart], dim=1)  # 3300000, 16+8+3=27
        x = self.layer4(x)  # 3300000, 1               # phi
        x = x.view(N, K)  # 100000, 33
        weights = F.softmax(x, 1)  # 100000, 33        # softmax
        return weights
