...
def total_loss(output, depth_gt):
    ...
    ones = torch.ones(depth_gt.size(0), 1, depth_gt.size(2),
                      depth_gt.size(3)).float().cuda()
    ...
    loss_depth = torch.log(torch.abs(output - depth_gt) + 0.5).mean()
    loss_dx = torch.log(torch.abs(output_grad_dx - depth_grad_dx) + 0.5).mean()
    loss_dy = torch.log(torch.abs(output_grad_dy - depth_grad_dy) + 0.5).mean()
    loss_normal = torch.abs(1 - cos(output_normal, depth_normal)).mean()

    loss = loss_depth + 0.5*loss_normal + 0.5*(loss_dx + loss_dy)
