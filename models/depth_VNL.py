...
class ModelLoss(object):
    def __init__(self, fx, fy, img_size):
        super(ModelLoss, self).__init__()
        self.virtual_normal_loss = VNL_Loss(focal_x=fx, focal_y=fy, input_size=img_size)
        # self.softmax = nn.Softmax(dim=1)

    def criterion(self, pred_depth, depth_gt):
        loss_normal = self.virtual_normal_loss(depth_gt, pred_depth)
        loss_depth = torch.log(torch.abs(pred_depth - depth_gt) + 0.5).mean()
        loss = 5 * loss_normal + loss_depth
        return loss
...
class VNL_Loss(nn.Module):
    ...
    def __init__(self, focal_x, focal_y, input_size,
        ...
        # self.pred_path = 'out/pred.ply'
        # self.gt_path = 'out/gt.ply'
    ...
    def select_points_groups(self, gt_depth, pred_depth):
        ...
        # pw_gt_np = pw_gt.numpy()
        # pw_gt_squeeze = pw_gt_np.squeeze()
        # print(pw_gt_squeeze.shape())
